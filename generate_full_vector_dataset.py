"""
generate_full_vector_dataset.py

Generates complete, high-fidelity vector_data.json for ganapati_python/ directly from:
1. assets/reference_image.png (Bappa single solid centerlines + eye pupil fills)
2. assets/mouse_reference.png (Mooshak mouse holding modak near Bappa's feet)

Guarantees:
- Single solid continuous lines (NO hollow double-line contours)
- High-fidelity eyes matching reference (almond shape + pupil fills + white highlights)
- Integrated mouse holding modak beside Bappa's lower leg/feet area
"""
...
def mouse_ref_to_canvas(mx, my, mouse_w=131.0, mouse_h=188.0):
    # Scale mouse to ~90px width and ~130px height in Bappa ref space, placed near right foot (x_ref ~ 300, y_ref ~ 380)
    scale_m = 0.65
    rx = 295.0 + (mx - mouse_w / 2.0) * scale_m
    ry = 385.0 + (my - mouse_h / 2.0) * scale_m
    return bappa_ref_to_canvas(rx, ry)

def smooth_points(pts_list, passes=1):
    if len(pts_list) <= 3:
        return pts_list
...
def generate():
    # 1. Process Bappa single solid centerline paths
    ...
    # Perform skeletonization to extract 1-pixel stroke centerlines
    ...
    for c in contours_bappa:
        ...
        smoothed = smooth_points(canvas_pts, passes=1)
    ...
    if mouse_img is not None:
        ...
        for c in contours_mouse:
            ...
            canvas_pts = [mouse_ref_to_canvas(p[0], p[1], wm, hm) for p in pts_mouse]
            smoothed = smooth_points(canvas_pts, passes=1)
            bezier_segs = fit_bezier_spline(smoothed)
            ...
            processed_paths.append({
                ...
                'step_order': 24,  # Final anatomical step: Mooshak mouse holding modak beside feet
                'bezier_segments': bezier_segs,
                'points': smoothed,
                'width': 5.0,
                'is_closed': False,
    ...
    print(f"Generated {len(processed_paths)} single solid vector paths (with Mooshak mouse) -> {OUT_JSON}")
